...
public class MyBirth {
	...
	/*
	 ...
	 oop(object oriantion project} 기술 중 하나 

	 - 외부에서 직접 변수에 접근할 수 없도록 맴버변수에 private 제한을 붙임.
	 */
	...
	public void setYear(int year) { //mainclass에 9번열로
		if(year < 1900 || year > 2023 ) {
			System.out.println("잘못된 연도 입력입니다.");
		} else {
			this.year = year; // 12열 year로 
			//그냥 year 쓰면 위에 int year 에 값이 들어가기때문에 this.year을 쓴다.
		}
    ...
    */
	...
	private boolean isValidateMonth(int day) {
		// 각 월에 적합한 일자가 세팅이 되어 있는지를 판단하는 메서드
		switch(this.month) {
		case 2 : 
			if(this.day > 28) {
				return false;
